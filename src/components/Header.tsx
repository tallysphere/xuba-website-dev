  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className='fixed inset-0 w-screen h-screen z-40 bg-xuba-purple-900 flex flex-col'
            initial='closed'
            animate='open'
            exit='closed'
          ...
          >
            ...
            <div className='absolute inset-0 bg-gradient-to-b from-xuba-purple-950/50 via-transparent to-xuba-purple-950/80 pointer-events-none' />

            {/* Main Navigation */}
            <nav className='flex-1 flex flex-col items-center justify-center z-10 px-6'>
                ...
                {navLinks.map((link, index) => (
                  ...
                  >
                    <Link
                      ...
                      onClick={toggleMenu}
                    >
                      <span className='text-white/40 text-sm font-light mr-4 group-hover:text-xuba-green-500 transition-colors duration-300'>
                        0{index + 1}
                      </span>
                      <span className='text-white text-3xl md:text-5xl font-extralight tracking-wide group-hover:text-xuba-green-400 transition-all duration-300 group-hover:tracking-wider'>
                        {link.name}
                      </span>
                    </Link>
                  </motion.div>
            ...
            <motion.div
              variants={linkVariants}
              className='relative z-10 px-6 md:px-16 py-8 border-t border-white/10'
            >
              <div className='max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6'>
                ...
                <div className='flex flex-col md:flex-row items-center gap-4 md:gap-8'>
                  <a
                    href='tel:0800332211'
                    className='flex items-center gap-2 text-white/90 hover:text-xuba-green-400 transition-colors text-sm'
                  >
