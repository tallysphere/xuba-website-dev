 ...
 * This renders the SEO title and meta description in the <head>.
 */
export async function generateMetadata(): Promise<Metadata> {
  const { data: aboutPage } = await sanityFetch({
    query: ABOUT_PAGE_QUERY,
    // Disable stega encoding for metadata to avoid invisible characters
    stega: false,
  })

  return {
    title: aboutPage?.seo?.title ?? 'About Us',
    ...
    openGraph: aboutPage?.seo?.image?.asset
      ...
      : undefined,
    robots: aboutPage?.seo?.noIndex ? { index: false, follow: false } : undefined,
  }
}
...
export default async function AboutPage() {
  ...
  const stats = aboutPage?.stats?.length
    ? aboutPage.stats.map((stat: { label: string | null; value: string | null; postfix: string | null }) => ({
        label: stat.label ?? '',
        value: stat.value ?? '0',
          ...
          <div className='px-4 sm:px-6 lg:px-8'>
            <div className='mx-auto max-w-7xl pt-16 sm:pt-24 lg:pt-36 pb-16 sm:pb-24 lg:pb-32'>
              <div className='mx-auto max-w-2xl gap-x-14 lg:mx-0 lg:flex lg:max-w-none lg:items-center'>
                ...
                {/* Hero image grid - uses images from About Page singleton */}
                  ...
                  <div className='mt-8 sm:mt-14 lg:mt-0 flex justify-center lg:justify-end gap-4 sm:gap-8 lg:pl-0'>
                      ...
                      {aboutPage.heroImages[0]?.asset && (
                        <div className='relative'>
                          <Image
                            ...
                            src={urlFor(aboutPage.heroImages[0]).width(400).height(528).fit('crop').url()}
                      ...
                      {aboutPage.heroImages[1]?.asset && (
                        <div className='relative'>
                          <Image
                            ...
                            src={urlFor(aboutPage.heroImages[1]).width(400).height(528).fit('crop').url()}
                            className='aspect-2/3 w-full rounded-xl bg-gray-900/5 object-cover shadow-lg'
                            width={400}
                            height={528}
                          />
                      ...
                      {aboutPage.heroImages[2]?.asset && (
                        <div className='relative'>
                          <Image
                            ...
                            src={urlFor(aboutPage.heroImages[2]).width(400).height(528).fit('crop').url()}
                            className='aspect-2/3 w-full rounded-xl bg-gray-900/5 object-cover shadow-lg'
                            width={400}
                            height={528}
                          />
                    ...
                    <div className='w-32 sm:w-44 flex-none space-y-4 sm:space-y-8 pt-16 sm:pt-32 lg:pt-0'>
                      {aboutPage.heroImages[3]?.asset && (
                        <div className='relative'>
                          <Image
                            ...
                            src={urlFor(aboutPage.heroImages[3]).width(400).height(528).fit('crop').url()}
                            className='aspect-2/3 w-full rounded-xl bg-gray-900/5 object-cover shadow-lg'
                            width={400}
                            height={528}
                          />
                      ...
                      {aboutPage.heroImages[4]?.asset && (
                        <div className='relative'>
                          <Image
                            alt={aboutPage.heroImages[4].alt ?? 'About us image'}
                            src={urlFor(aboutPage.heroImages[4]).width(400).height(528).fit('crop').url()}
                            className='aspect-2/3 w-full rounded-xl bg-gray-900/5 object-cover shadow-lg'
                            width={400}
                            height={528}
                          />
