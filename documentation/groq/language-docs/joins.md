...
Antijoins are similar to semijoins, except they check whether the document in the left relation does _not_ match any documents in the right relation on the join condition. For example, the following query fetches all employees that do not belong to any department:
